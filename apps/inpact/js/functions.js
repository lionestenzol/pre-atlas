...
function exportState() {
  ...
  link.href = url;
  link.download = `inpact-backup-${new Date().toISOString().slice(0, 10)}.json`;
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);

  // Track export statistics
  localStorage.setItem('inpact-last-export', new Date().toISOString());
  const exportCount = parseInt(localStorage.getItem('inpact-export-count') || '0');
  localStorage.setItem('inpact-export-count', (exportCount + 1).toString());
  ...
  reader.onload = function(e) {
    try {
      ...
      if (confirm(confirmMsg)) {
        ...
        localStorage.setItem('inpact-last-import', new Date().toISOString());
      ...
      UI.showToast('Import Failed', 'The file is not a valid inPACT backup', 'error');
    }
  };
