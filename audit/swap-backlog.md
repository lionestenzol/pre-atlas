...
| 7 | `services/uasc-executor/server.py` | **fastapi** | HIGH | ~64 (handlers) | ✅ **SHIPPED 2026-05-30 · Session 3** · 11/11 TestClient smokes pass · HMAC via `Depends(verify_hmac)` on /exec only (3 GETs stay open) · raw-body parity preserved through `await request.body()` in dependency · CORS `*` + legacy `{"error": ...}` body shape preserved via one-line middleware + `StarletteHTTPException` handler |
