...
Write-Host "`n[1/5] Starting Delta API..." -ForegroundColor Yellow
...
# Start UASC Executor in new window
Write-Host "[2/6] Starting UASC Executor..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$RepoRoot\services\uasc-executor'; python server.py --port 3008"

# Start Cortex execution layer in new window
Write-Host "[3/6] Starting Cortex Execution Layer..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$RepoRoot\services\cortex'; python -m uvicorn cortex.main:app --host 0.0.0.0 --port 3009"

# Wait for APIs to initialize
Start-Sleep -Seconds 5

# Run cognitive refresh
Write-Host "[4/6] Running Cognitive Sensor..." -ForegroundColor Yellow
python "$RepoRoot\services\cognitive-sensor\refresh.py"

# Build daily projection
Write-Host "[5/6] Building Daily Projection..." -ForegroundColor Yellow
python "$RepoRoot\services\cognitive-sensor\build_projection.py"

# Push to Delta
Write-Host "[6/6] Pushing to Delta..." -ForegroundColor Yellow
...
Write-Host "Cortex Executor:  http://localhost:3009" -ForegroundColor White
Write-Host "UASC Executor:    http://localhost:3008" -ForegroundColor White
...
Write-Host "CycleBoard:       $RepoRoot\services\cognitive-sensor\cycleboard\index.html" -ForegroundColor White
