...
def open_conversation():
    db = load_db()
    title = input("Exact conversation title: ").strip()
    for c in db:
        if c["title"] == title:
            print("\n" + "=" * 80)
            for m in c["messages"]:
...
def main():
    while True:
        ...
        print("3) Open a conversation")
        print("4) Find unfinished goals")
        print("5) Exit")
        choice = input("> ").strip()

        if choice == "1": build_db()
        elif choice == "2": search_db()
        elif choice == "3": open_conversation()
        elif choice == "4": find_unfinished()
