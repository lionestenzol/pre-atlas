        ...
        for t in triggers:
            patterns.append(Pattern(
                ...
                description=(
                    ...
                    f"{t['bad']}/{t['total']} times "
                    f"({t['ratio']:.0%})"
                ),
                confidence=t["ratio"],
                evidence=[
                    f"{t['bad']} of {t['total']} conversations "
        ...
        for t in catalysts:
            patterns.append(Pattern(
                ...
                description=(
                    ...
                    f"{t['good']}/{t['total']} times "
                    f"({t['ratio']:.0%})"
                ),
                confidence=t["ratio"],
                evidence=[
                    f"{t['good']} of {t['total']} conversations "
                ...
                if c["outcome"] in ("abandoned", "looped"):
                    ...
                    if len(run) >= 3:
                        patterns.append(Pattern(
                            ...
                            data={"topic": topic, "run_length": len(run), "convo_ids": [c["id"] for c in run]},
                        ))
                    run = []
            if len(run) >= 3:
                patterns.append(Pattern(
