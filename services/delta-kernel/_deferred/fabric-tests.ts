...
export async function testDriftImmunity(): Promise<TestResult> {
  ...
  try {
    ...
    // RECOVER → CLOSURE requires sleep OK
    ...
    // CLOSURE → BUILD requires open_loops LOW
    ...
    const fromCloseLoopsMode = route('CLOSURE', closeLoopsSignals);
    const canEnterBuild = fromCloseLoopsMode === 'BUILD';
    details.push(
      `With loops closed: CLOSURE → ${fromCloseLoopsMode} (${canEnterBuild ? 'can enter BUILD' : 'blocked'})`
    );
