...
| OQ-17 | ~~`Signal.v1.source_layer` enum does not include `"droplist"`. PKT-005 uses `"optogon"` as a placeholder. Should the enum be extended?~~ | **RESOLVED by Stop 5** (2026-06-17). Both `Signal.v1.source_layer` and sibling `AtlasArtifact.v1.source_layer` enums extended with `"droplist"`. `atlas_signal.dag_to_signal()` default flipped from `"optogon"` → `"droplist"`. Bruke override — shipped for honesty even though no consumer branches on the value yet; `lattice-projection.ts:200-202` still filters on the `dag_id` marker, which remains stable across the enum widening (set-monotone). |
...
| `source_layer` | `"droplist"` (Signal.v1 enum extended by Stop 5, 2026-06-17) |
...
- ~~**OQ-17** extends `Signal.v1.source_layer` enum to include `"droplist"` once Atlas-side cares about distinguishing.~~ **RESOLVED by Stop 5 (2026-06-17).** Both `Signal.v1` and `AtlasArtifact.v1` enums now include `"droplist"`; `atlas_signal.dag_to_signal()` default flipped. `lattice-projection.ts:200-202` continues to filter on the `dag_id` marker (the widening doesn't change consumer behavior, by design).
